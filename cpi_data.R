...
ggplot(data = ch_plot, aes(x=Year, y=RPPP_Score))+
  geom_point(aes(color="FF0000"), size=3)+
  ylab("RPPP Score")+
...
# Plot of RPPP for the free floating club:
...
ggplot(data = uk_plot, aes(x=Year,y=RPPP_Score, colour=Country))+
  geom_point(size=3)+
  scale_colour_manual(values = c("#001489","#138808"))+
  ylab("RPPP Score")+
...
rpvstp <- rbind(uk_l_plot,in_l_plot,fr_l_plot,gr_l_plot,ch_l_plot)

# Plot 
ggplot(data = rpvstp, aes(x=Time_differential,y=RPPP_score, colour=Country))+
  #geom_point()+
  geom_line()+
  scale_colour_manual(values = c("#001489","#138808","purple","#FFCE00","#DD0000"))+
  ylab("RPPP Score")+
